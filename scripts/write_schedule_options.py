...
p = Path(__file__).resolve().parents[1] / "src/components/create-post/SchedulePostOptions.jsx"
...
function ScheduleCalendar({ selectedDate, onSelectDate, minDate }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-700 dark:bg-slate-950/50">
      <motion className="mb-2 flex items-center justify-between">
        <button
          type="button"
...
export default function SchedulePostOptions({
  ...
  return (
    <section className="overflow-hidden rounded-xl border-2 border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center gap-2 border-b border-slate-100 bg-slate-50/80 px-4 py-3 dark:border-slate-800 dark:bg-slate-950/40">
...
content = content.replace('<motion', '<motion').replace('</motion>', '</motion>')
content = content.replace('<motion', '<div').replace('</motion>', '</motion>')
